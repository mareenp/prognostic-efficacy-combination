...
df <- map_dfr(results, .id = "condition", ~.x$prob) %>% 
  pivot_longer(cols = c(Treat_New, Treat_Split, Treat_Ref, Treat_New_CV), names_to = "design", values_to = "estimate") %>% 
  ...
  mutate(design = ifelse(design == "Treat_Split", "Split of control into training and test (Split)", 
                      ifelse(design == "Treat_Ref", "External biomarker study (Reference)",
                             ifelse(design == "Treat_New", "Prognostic-efficacy-combination design (New)",
                             "Prognostic-efficacy-combination design with CV (New-CV)")))) %>% 
...
plots <- map(as.list(unique(df$parameter)),
      function(x) {
        df %>% filter(parameter == x) %>%
          ...
          geom_boxplot(colour = "black", alpha = 0.75, lwd = 0.5, median.linewidth = 0.9,
                       outlier.shape = NA) +
...
png(filename = paste("Plot_bias_CV.png"), 
     width = 6500, height = 5500, res = 650)
 (plots[[1]] | plots[[2]]) / 
 (plots[[3]] | plots[[4]]) / 
  legend + plot_layout(heights = c(1,1,0.4))
dev.off()
